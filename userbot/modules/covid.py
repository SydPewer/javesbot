...
async def corona(event):
    ...
    if country_data:
        output_text =  f"`Confirmed`   :  <b> {country_data['confirmed']}\n"
        output_text += f"`Active`          :   {country_data['active']}\n"
        output_text += f"`Deaths`          :   {country_data['deaths']}\n"
        output_text += f"`Recovered`   :   {country_data['recovered']}</b>\n\n"        
        output_text += f"---------TOTAL----------\n\n"                
        output_text += f"`Deaths`          : <b>  {covid.get_total_deaths()}\n"
        output_text += f"`Recovered`   :   {covid.get_total_recovered()}\n"
        output_text += f"`Confirmed`   :   {covid.get_total_confirmed_cases()}\n"
        output_text += f"`Active`          :   {covid.get_total_active_cases()}</b>\n\n"
