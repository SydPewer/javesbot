...
@bot.on(javes_cmd(pattern="yt(a|v) (.*)"))
...
async def download_video(v_url):
    ...
    event = await eor(v_url, "`Preparing to download sur...`")
    ...
    try:
        await event.edit("**Fetching YT link sur...**")
        ...
        await eod(event, "`The download content was too short sur.`")
        return
    except GeoRestrictedError:
        await eod(event, 
            "`Video is not available from your geographic location due to geographic restrictions imposed by a website, me failed sur.`"
        )
        return
    except MaxDownloadsReached:
        await eod(event, "`Max-downloads limit has been reached sur.`")
        return
    except PostProcessingError:
        await eod(event, "`There was an error during post processing sur.`")
        return
    except UnavailableVideoError:
        await eod(event, "`Media is not available in the requested format sur.`")
        return
    except XAttrMetadataError as XAME:
        await edit_or_reply(v_url, f"`{XAME.code}: {XAME.msg}\n{XAME.reason}`")
        return
    except ExtractorError:
        await eod(event, "`There was an error during info extraction sur.`")
        return
    ...
    if song:
        await eor(event, 
            f"📤 `Preparing to upload audio sur:`\
        \n\n**{ytdl_data['title']}**\
...
CmdHelp("youtube").add_command(
  ...
  "Youthoob ki duniya ProfessorSeBantiHai #MePro."
).add_warning(
  "✅ Harmless Module."
).add()
